"""
Temporal Maze — Multi-Agent Negotiation Edition  (UI v4 🎮 Multiplayer)
=========================================================
pip install pygame
python temporal_maze.py

✨ NEW: LOCAL MULTIPLAYER MODE!
• 3 players compete on same keyboard
• Unique key bindings per player
• Real-time race to the exit
• Player stats & winner tracking
"""
...
C = dict(
    ...
    r0          =(255, 200,   0),    # Player 1 / Robot 0 - Gold
    r1          =( 70, 200, 255),    # Player 2 / Robot 1 - Cyan  
    r2          =(185,  75, 255),    # Player 3 / Robot 2 - Violet
    # ✨ Gradients & effects
...
# ── PLAYER KEY BINDINGS FOR LOCAL MULTIPLAYER ─────────────────────────────────
PLAYER_KEYS = {
    0: {  # Player 1 - WASD + Space
        "up": pygame.K_w, "down": pygame.K_s, 
        "left": pygame.K_a, "right": pygame.K_d,
        "wait": pygame.K_SPACE, "name": "Player 1"
    },
    1: {  # Player 2 - Arrow Keys + Right Shift
        "up": pygame.K_UP, "down": pygame.K_DOWN,
        "left": pygame.K_LEFT, "right": pygame.K_RIGHT, 
        "wait": pygame.K_RSHIFT, "name": "Player 2"
    },
    2: {  # Player 3 - IJKL + Enter
        "up": pygame.K_i, "down": pygame.K_k,
        "left": pygame.K_j, "right": pygame.K_l,
        "wait": pygame.K_RETURN, "name": "Player 3"
    }
}
...
SETTINGS_OPTIONS = [
    ("Show Hints", "show_hint", "H", "Toggle hint overlay"),
    ("Particle FX", "particle_fx", "P", "Celebration particles"),
    ("Show FPS", "show_fps", "F", "Display frame rate"),
    ("Debug Mode", "debug_mode", "D", "Extra diagnostics"),
]
...
def settings_button_rect(W):
    return pygame.Rect(W - 40, HUD_H + 8, 32, 32)


def settings_panel_rect(W, H):
    return pygame.Rect((W - 340) // 2, (H - 260) // 2 - 20, 340, 260)


def settings_row_rects(W, H):
    panel = settings_panel_rect(W, H)
    return [
        pygame.Rect(panel.x + 12, panel.y + 40 + i * 46, panel.w - 24, 38)
        for i in range(len(SETTINGS_OPTIONS))
    ]


def handle_settings_click(s: "GS", mx, my, W, H):
    gear = settings_button_rect(W)
    ...
    for row_rect, (_, attr, _, _) in zip(settings_row_rects(W, H), SETTINGS_OPTIONS):
...
def robot_starts(n, g=None, tm=None, T=None):
    ...
    def can_spawn(r, c):
        ...
        if g[r][c]:
            return False
        if tm is not None and T is not None and 0 in tm[r][c]:
            return False
        return True

    for r, c in cands:
        if can_spawn(r, c) and (r, c) not in seen:
            valid.append((r, c))
            seen.add((r, c))
        if len(valid) == NUM_R:
            return valid
...
def negotiate(g, tm, n, T):
    ...
    starts=robot_starts(n, g, tm, T); paths=[]
...
# ── Enhanced State with Multiplayer Support ───────────────────────────────────
@dataclass
class GS:
    ...
    show_end_page: bool = False
    screen_shake: tuple = (0, 0)
    shake_timer: int = 0
    debug_mode: bool = False
    # 🎮 LOCAL MULTIPLAYER FIELDS
    mp_players: list = field(default_factory=lambda: [
        {"r": 0, "c": 0, "t": 0, "steps": 0, "waits": 0, "done": False, "trail": [], "winner_order": 0}
        for _ in range(NUM_R)
    ])
    mp_winner: int = -1
    mp_rankings: list = field(default_factory=list)  # Order players finished
    mp_collisions: list = field(default_factory=list)  # Track player collisions
...
def new_level(s:GS):
    ...
    starts = robot_starts(s.n, s.grid, s.temporal, s.T)
    s.ma_mode=False; s.ma_paths=[]; s.ma_step=0
    s.ma_pos=list(starts)
    ...
    s.show_end_page = False
    s.show_settings = False
    s.screen_shake = (0, 0)
    s.shake_timer = 0
    # 🎮 Reset multiplayer fields
    s.mp_players = [
        {"r": start[0], "c": start[1], "t": 0, "steps": 0, "waits": 0, 
         "done": False, "trail": [], "winner_order": 0}
        for start in starts
    ]
    s.mp_winner = -1
    s.mp_rankings = []
...
def draw_parallax_bg(surf, W, H, frame):
    ...
    for i in range(-2, W//grid_spacing + 3):
        ...
        pygame.draw.line(surf, (*C["grid_line"], alpha), (x, 0), (x, H), width=1)
...
def draw_legend(surf, x, y, W_panel, frame):
    panel_h = len(LEGEND_ITEMS) * 24 + 24
    drect_animated(surf, C["panel"], (x, y, W_panel, panel_h), r=8,
          border=1, bcol=C["border"], frame=frame)
    dtext(surf, "Maze Legend", F_UI, C["text2"], x+W_panel//2, y+6, "center")
    dtext(surf, "Tile meanings", F_SM, C["text3"], x+W_panel//2, y+20, "center")
    for i,(col,pattern,label) in enumerate(LEGEND_ITEMS):
        ry = y + 30 + i * 24
        drect_animated(surf, C["panel2"], (x+6, ry-2, W_panel-12, 20), r=5, border=1, bcol=C["border"], frame=frame)
        drect_animated(surf, col, (x+12, ry+1, 14, 14), r=3, frame=frame)
        if pattern:
            draw_cell_pattern(surf, pattern, x+12, ry+1, 14)
        dtext(surf, label, F_SM, C["text2"], x+34, ry+3)


# ── Enhanced Controls Panel with Multiplayer Keys ─────────────────────────────
def get_control_rows(s:GS):
    if s.mode == "player":
        return [
            ("Move", "Arrows or WASD", C["text2"]),
            ("Wait", "SPACE", C["accent"]),
            ("Hint", "H", C["accent"]),
            ("Time view", "0-9 keys or time pills", C["text2"]),
            ("Mouse", "Click adjacent cell / time pills", C["text2"]),
            ("Settings", "LALT", C["warn"]),
            ("FPS / Debug", "F / D", C["text2"]),
            ("Restart / Menu / Quit", "R / ESC / Q", C["text2"]),
        ]
    if s.mode == "ai":
        return [
            ("Run solver", "A", C["accent"]),
            ("Show result page", "E after finish", C["green"] if s.won else C["text3"]),
            ("Time view", "Click time pills", C["text2"]),
            ("Settings", "LALT", C["warn"]),
            ("FPS / Debug", "F / D", C["text2"]),
            ("Reset / Menu / Quit", "R / ESC / Q", C["text2"]),
        ]
    if s.mode == "multi":
        return [
            ("Run race", "A", C["accent"]),
    ...
    if s.mode == "local_mp":
        return [
            ("P1 Move / Wait", "WASD / SPACE", C["r0"]),
            ("P2 Move / Wait", "Arrows / RSHIFT", C["r1"]),
            ("P3 Move / Wait", "IJKL / ENTER", C["r2"]),
            ("Time view", "Click time pills", C["text2"]),
            ("Settings", "LALT", C["warn"]),
            ("FPS / Debug", "F / D", C["text2"]),
            ("Restart / Menu / Quit", "R / ESC / Q", C["text2"]),
...
def draw_controls_panel(surf, x, y, W_panel, s:GS, frame):
    rows = get_control_rows(s)
    row_h = 24
    ph = len(rows) * row_h + 32
    ...
    dtext(surf, "Active in this mode", F_SM, C["text3"], x+W_panel//2, y+20, "center")
    for i, (action, keys, col) in enumerate(rows):
        ry = y + 34 + i * row_h
        drect_animated(surf, C["panel2"], (x+6, ry-2, W_panel-12, 20), r=5, border=1, bcol=C["border"], frame=frame)
        dtext(surf, action, F_SM, col, x+12, ry+2)
        dtext(surf, keys, F_SM, C["text2"], x+W_panel-12, ry+2, "topright")


# ── Multiplayer Player Panel ──────────────────────────────────────────────────
# ── Multiplayer Player Panel (FIXED) ──────────────────────────────────────────
# ── Robot Panel for AI Multi-Agent Mode ──────────────────────────────────────
def draw_robot_panel(surf, x, y, W_panel, s:GS, frame):
    """Draw robot status panel for multi-agent AI mode"""
    ph = NUM_R*38+20
    drect_animated(surf, C["panel"], (x,y,W_panel,ph), r=8, border=1, bcol=C["border"], frame=frame)
    ...
    for rid in range(NUM_R):
        ...
        if s.ma_done[rid]:
            ...
            scol = C["text2"]
        elif s.ma_phase in ("negotiate", "scout", "gossip"):
            nsteps = len(s.ma_paths[rid]) if rid<len(s.ma_paths) and s.ma_paths[rid] else 0
            status = f"📋 Plan: {nsteps} steps" if nsteps else "⏳ Planning..."
            scol = C["text2"]
        else:
            status = "⏳ Waiting"
            scol = C["text3"]
...
def draw_mp_player_panel(surf, x, y, W_panel, s:GS, frame):
    """Draw player status panel for local multiplayer mode"""
    ph = NUM_R*42+24
    drect_animated(surf, C["panel"], (x,y,W_panel,ph), r=8, border=1, bcol=C["border"], frame=frame)
    ...
    for pid in range(NUM_R):
        ...
        pygame.draw.circle(surf, col, (x+20, ry+16), 10)
        pygame.draw.circle(surf, (0,0,0), (x+20, ry+16), 10, 1)
        # Winner crown
        if player["done"] and pid == s.mp_winner:
            for angle in range(0, 360, 60):
                rad = 14
                px = x+20 + int(rad * math.cos(math.radians(angle + frame*2)))
                py = ry+16 + int(rad * math.sin(math.radians(angle + frame*2)))
                pygame.draw.circle(surf, C["r0"], (px, py), 3)
        
        # Player name + key hints (FIXED: use pygame.key.name())
        dtext(surf, f"{keys['name']}", F_SM, col, x+36, ry+4)
        # ✅ Use pygame.key.name() to convert key int to string
        up_key_name = pygame.key.name(keys['up']).upper()
        dtext(surf, f"[{up_key_name}]", F_SM, C["text3"], x+36, ry+16)
        ...
        if player["done"]:
            if pid == s.mp_winner:
                ...
                place = s.mp_rankings.index(pid) + 2
                if place == 2:
                    status = "🥈 2nd"
                elif place == 3:
                    status = "🥉 3rd"
                    ...
                    status = f" {place}th"
                scol = C["warn"] if place==2 else C["text3"]
            else:
                status = "✓ Finished"
                scol = C["text3"]
        else:
            status = f"⚡ {player['steps']} steps"
            scol = C["text2"]
        dtext(surf, status, F_SM, scol, x+36, ry+28)
        
        # Key badge for wait (FIXED: use pygame.key.name())
        wait_key_name = pygame.key.name(keys['wait']).upper()
        draw_key_badge(surf, wait_key_name, x+W_panel-50, ry+8, frame=frame)

# ── Enhanced Maze Renderer with Multiplayer ───────────────────────────────────
def draw_maze(surf, s:GS, ox, oy, cell, frame):
    ...
    if s.ma_phase:
        for rid in range(NUM_R):
            ...
            for i, (tr2, tc2) in enumerate(trail):
                ...
                pygame.draw.rect(ts2, (*col, alpha), (0, 0, size, size), border_radius=size//4)
    ...
    # 🎮 LOCAL MULTIPLAYER: Draw all 3 human players
    if s.mode == "local_mp":
        ...
        for pid in range(NUM_R):
            ...
            for i, (tr, tc) in enumerate(player["trail"]):
                age_ratio = i / max(1, len(player["trail"]))
                alpha = int(30 + 50 * age_ratio)
                ts = pygame.Surface((cell-4, cell-4), pygame.SRCALPHA)
                ts.fill((*col, alpha))
                surf.blit(ts, (ox + tc*cell + 2, oy + tr*cell + 2))
        
        # Draw player robots with labels
        for pid in range(NUM_R):
            player = s.mp_players[pid]
            col = RCOLS[pid]
            cx = ox + player["c"]*cell + cell//2
            cy = oy + player["r"]*cell + cell//2
            
            # Pulsing glow for active players
            glow = 0.2 * math.sin(frame * 0.3 + pid * 2) if not player["done"] else 0
            tri_size = max(8, int(cell*0.7 + glow*3))
            
            tri(surf, col, cx, cy, tri_size, str(pid+1), af, glow=(not player["done"]), frame=frame)
            
            # Winner indicator
            if player["done"] and pid == s.mp_winner:
                # Draw crown animation
                for angle in range(0, 360, 72):
                    rad = cell//2 + int(4 * math.sin(frame * 0.4 + angle))
                    px = cx + int(rad * math.cos(math.radians(angle)))
                    py = cy - int(rad * math.sin(math.radians(angle))) + cell//4
                    pygame.draw.circle(surf, C["r0"], (px, py), 4)
            
            # Collision indicator
            if (player["r"], player["c"]) in s.mp_collisions:
                pygame.draw.circle(surf, C["paradox"], (cx, cy), cell//2 + 2, width=2)

    # Draw collision markers
    for (cr, cc) in s.mp_collisions:
        pulse = 0.4 * (0.5 + 0.5 * math.sin(frame * 0.4))
        cf = pygame.Surface((cell, cell), pygame.SRCALPHA)
        pygame.draw.circle(cf, (*C["paradox"], int(180 + pulse*40)), (cell//2, cell//2), cell//2 - 2)
        surf.blit(cf, (ox + cc*cell, oy + cr*cell))


# ── Enhanced HUD with Multiplayer Stats ───────────────────────────────────────
HUD_H = 52
...
def draw_hud(surf, s:GS, W, frame):
    ...
    if s.mode == "local_mp":  # 🎮 Multiplayer HUD
        ...
        for pid in range(NUM_R):
            ...
            dtext(surf, f"P{pid+1}", F_SM, col, x+12, 12)
            dtext(surf, f"Steps: {player['steps']}", F_SM, C["text"], x+12, 26)
            if player["done"]:
                place = "🏆 1st" if pid == s.mp_winner else f"{s.mp_rankings.index(pid)+2}nd" if s.mp_rankings.index(pid)+2==2 else f"{s.mp_rankings.index(pid)+2}rd" if s.mp_rankings.index(pid)+2==3 else f"{s.mp_rankings.index(pid)+2}th" if pid in s.mp_rankings else "DNF"
        ...
        for lbl,val in stats:
            drect_animated(surf, C["panel2"], (x,8,56,36), r=6, border=1, bcol=C["border"], frame=frame)
        ...
        if s.ma_winner >= 0:
            ...
            drect_animated(surf, C["win_bg"], (165,8,170,36), r=8, border=1, bcol=wc, pulse=True, frame=frame)
    ...
    for t in range(s.T):
        ...
        if glow_size > 0:
            ...
            pygame.draw.rect(glow_surf, (*C["accent"][:3], 50), 
                           (0,0,40+glow_size*2,22+glow_size*2), border_radius=11+glow_size//2)
            surf.blit(glow_surf, (px-glow_size, 24-glow_size))
        drect_animated(surf, bg_color, (px, 24, 40, 22), r=11, border=2, 
                      bcol=C["accent"] if on else C["border2"], pulse=on, frame=frame)
...
def draw_statusbar(surf, s:GS, W, H, frame):
    ...
    if s.msg and s.msg_timer > 0:
        ...
        parts = [f"[{keys}] {label}" for label, keys, _ in get_control_rows(s)[:4]]
        hint = " · ".join(parts)
...
def draw_log_panel(surf, s:GS, x, y, w, frame):
    ...
    for i, ln in enumerate(lines):
        if "WINNER" in ln:
            col = C["green"]; pulse = frame % 30 < 15
        elif "PARADOX" in ln:
            col = C["paradox"]; pulse = frame % 20 < 10
        elif ln.startswith("Phase"):
            col = C["text2"]; pulse = False
        else:
            col = C["text3"]; pulse = False
        max_chars = w // 7
...
def draw_hint(surf, s:GS, W, H, frame):
    ...
    pw = min(W-60, 540); ph = len(lines)*20 + 60
    px = (W-pw)//2; py = (H-ph)//2
    drect_animated(surf, (0,0,0), (0,0,W,H), a=140, frame=frame)
    drect_animated(surf, C["panel"], (px, py, pw, ph), r=12, border=2, bcol=C["accent"], pulse=True, frame=frame)
    dtext(surf, "💡 HINT", F_HEAD, C["accent"], px+pw//2, py+12, "center", pulse=True, frame=frame)
    pygame.draw.line(surf, C["border"], (px+12, py+34), (px+pw-12, py+34))
    for i, ln in enumerate(lines):
        fade_alpha = min(255, int((frame - i*5) / 10 * 255)) if frame > i*5 else 0
        if fade_alpha > 0:
            dtext(surf, ln, F_UI, (*C["text"][:3], fade_alpha), px+16, py+42+i*20)
    dtext(surf, "Press [H] to close", F_SM, C["text3"], px+pw//2, py+ph-20, "center", pulse=True, frame=frame)


# ── Win Overlay with Multiplayer Support ──────────────────────────────────────
def draw_win(surf, s:GS, W, H, frame):
    if not s.won or (s.mode in ("ai", "multi") and not s.show_end_page): return
    ...
    if s.particle_fx:
        ...
        for p in s.particles:
            p.draw(surf)
    pw = min(W-40, 520); ph = 220 if s.mode in ("player", "local_mp") else 260
    px = (W-pw)//2; py = (H-ph)//2
    ...
    if s.mode == "player":
        ...
        dtext(surf, "🎉 EXIT REACHED!", F_TITLE, C["win_border"], px+pw//2, py+18, "center", shadow=True, pulse=True, frame=frame)
        pygame.draw.line(surf, C["border"], (px+20, py+54), (px+pw-20, py+54))
        stat_data = [("STEPS", str(s.steps)), ("WAITS", str(s.waits)), ("OPTIMAL", str(s.par)), ("EFF", f"{eff}%")]
        bw = (pw-40)//4
        ...
        dtext(surf, "[N] Next Level    [R] Retry    [ESC] Menu", F_UI, C["text3"], px+pw//2, py+196, "center", pulse=True, frame=frame)
    
    elif s.mode == "local_mp":  # 🎮 Multiplayer win screen
        winner = s.mp_winner
        dtext(surf, "🏁 RACE COMPLETE!", F_TITLE, C["win_border"], px+pw//2, py+18, "center", shadow=True, pulse=True, frame=frame)
        pygame.draw.line(surf, C["border"], (px+20, py+54), (px+pw-20, py+54))
        
        if winner >= 0:
            wc = RCOLS[winner]
            drect_animated(surf, C["panel2"], (px+pw//2-100, py+64, 200, 55), r=10, border=2, bcol=wc, pulse=True, frame=frame)
            dtext(surf, f"🏆 {PLAYER_KEYS[winner]['name']} WINS!", F_HEAD, wc, px+pw//2, py+78, "center", shadow=True, pulse=True, frame=frame)
            dtext(surf, f"{RNAMES[winner]} Team", F_SM, C["text3"], px+pw//2, py+102, "center")
        
        # Player rankings
        dtext(surf, "📊 Final Rankings:", F_UI, C["text2"], px+20, py+135)
        for rank, pid in enumerate([s.mp_winner] + [p for p in range(NUM_R) if p != s.mp_winner and p in s.mp_rankings]):
            ...
            dtext(surf, f"{medal} {rank+1}. {PLAYER_KEYS[pid]['name']}: {player['steps']} steps", 
                  F_SM, col if rank==0 else C["text2"], px+20, py+155+rank*18)
        
        dtext(surf, "[N] Next    [R] Retry    [ESC] Menu", F_UI, C["text3"], px+pw//2, py+ph-22, "center")
    
    else:  # AI/Multi-agent mode
        w = s.ma_winner
        title = "🏁 RACE OVER!" if w>=0 else "✅ RACE COMPLETE"
        dtext(surf, title, F_TITLE, C["win_border"], px+pw//2, py+18, "center", shadow=True, pulse=True, frame=frame)
        pygame.draw.line(surf, C["border"], (px+20, py+54), (px+pw-20, py+54))
        if w >= 0:
            wc = RCOLS[w]
            drect_animated(surf, C["panel2"], (px+pw//2-110, py+64, 220, 60), r=10, border=2, bcol=wc, pulse=True, frame=frame)
            pygame.draw.circle(surf, wc, (px+pw//2-70, py+94), 18)
            dtext(surf, str(w), F_HEAD, (20,20,20), px+pw//2-70, py+94, "center")
            dtext(surf, f"🤖 ROBOT {w} WINS!", F_HEAD, wc, px+pw//2+10, py+80, "center", shadow=True, pulse=True, frame=frame)
        ...
        for rid in range(NUM_R):
            ...
            dtext(surf, f"{status} · {plen} steps", F_SM, scol, px+210, ry+5, pulse=(rid==w), frame=frame)
        dtext(surf, "[N] Next    [R] Retry    [ESC] Menu", F_UI, C["text3"], px+pw//2, py+ph-22, "center")
...
def draw_title(surf, W, H, sel_diff, sel_mode, hover, frame):
    ...
    drect_animated(surf, C["panel"], (W//2-265, 18, 530, 100), r=16, border=1, bcol=C["border2"], frame=frame)
    dtext(surf, "TEMPORAL", F_TITLE, C["accent"], W//2, 28, "center", shadow=True, pulse=True, frame=frame)
    dtext(surf, "MAZE", F_TITLE, C["text"], W//2, 64, "center", shadow=True, pulse=True, frame=frame)
    dtext(surf, "Multi-Agent puzzle solving and local multiplayer racing", F_UI, C["text3"], W//2, 102, "center", pulse=True, frame=frame)
    pygame.draw.line(surf, C["border"], (W//2-160, 118), (W//2+160, 118))

    tw_cards = [
        (C["r0"],  "🎮 Move Through Space", "Use Arrow keys or WASD\nto navigate the grid.\nReach the RED exit cell."),
        (C["cell_temp"], "⏱️  Time Walls", "Amber cells are blocked\nONLY at certain time steps.\nThey open as time advances!"),
        (C["accent"], "🔄 Wait = Time Travel", "Press SPACE to stay in\nplace. This advances time\nby 1 — opening new paths."),
        (C["r2"],  "🤖 Multi-Agent Race", "3 robots Scout → Gossip →\nNegotiate paths → Race\nto the exit simultaneously."),
    ]
    cw2 = int((W-60)//4); ch2 = 115
    for i, (icol, title2, body) in enumerate(tw_cards):
        cx2 = 20 + i*(cw2+6); cy2 = 130
        on = (hover == f"card{i}")
        bg = (C["panel2"], C["panel"]) if on else C["panel"]
        bc = C["accent"] if on else C["border"]
        drect_animated(surf, bg, (cx2, cy2, cw2, ch2), r=10, border=2 if on else 1, bcol=bc, hover=on, frame=frame)
        bar_w = cw2 + int(5 * math.sin(frame * 0.1 + i))
        drect_animated(surf, icol, (cx2, cy2, bar_w, 5), r=3, frame=frame)
        dtext(surf, title2, F_UI, C["text"], cx2+cw2//2, cy2+14, "center", shadow=on, pulse=on, frame=frame)
        ...
        for j, line in enumerate(body.split("\n")):
            ...
            dtext(surf, line, F_SM, (*C["text3"][:3], fade), cx2+cw2//2, cy2+40+j*18, "center")

    diffs = list(DIFFS.keys())
    drect_animated(surf, C["panel"], (12, 244, W-24, 118), r=12, border=1, bcol=C["border"], frame=frame)
    dtext(surf, "SELECT DIFFICULTY", F_UI, C["text3"], W//2, 255, "center")
    pygame.draw.line(surf, C["border"], (W//2-100, 271), (W//2+100, 271))
    dcw = int((W-40)//4); dch = 78
    for i, d in enumerate(diffs):
        dcx = 20 + i*(dcw+5); dcy = 277
        selected = (d == sel_diff)
        hovered = (hover == f"diff_{d}")
        active = selected or hovered
        bg = (C["accent2"], C["panel"]) if active else C["panel"]
        bc = C["accent"] if active else C["border"]
        drect_animated(surf, bg, (dcx, dcy, dcw, dch), r=10, border=2 if active else 1, bcol=bc, hover=hovered, pulse=selected, frame=frame)
        info = DIFFS[d]
        dtext(surf, d, F_HEAD, C["text"] if active else C["text2"], dcx+dcw//2, dcy+8, "center", shadow=active, pulse=selected, frame=frame)
        dtext(surf, f"{info['n']}×{info['n']}", F_UI, C["accent"] if active else C["text3"], dcx+dcw//2, dcy+30, "center")
        dtext(surf, f"{info['T']} time slices", F_SM, C["text3"], dcx+dcw//2, dcy+48, "center")
        dtext(surf, f"{int(info['tw']*100)}% temporal walls", F_SM, C["text3"], dcx+dcw//2, dcy+63, "center")

    # 🎮 UPDATED MODE SELECTOR WITH LOCAL MULTIPLAYER
    drect_animated(surf, C["panel"], (12, 352, W-24, 138), r=12, border=1, bcol=C["border"], frame=frame)
    ...
    modes = [
        ...
        ("local_mp", "👥 LOCAL MULTIPLAYER", "🎮 3 players on one keyboard!\nRace to the exit first", "WASD | Arrows | IJKL"),  # ✨ NEW
    ]
    mw = int((W-40)//4); mh = 98
    for i, (m, mname, mdesc, mhint) in enumerate(modes):
        mcx = 20 + i*(mw+5); mcy = 384
        selected = (m == sel_mode)
        hovered = (hover == f"mode_{m}")
        active = selected or hovered
        bg = (C["accent2"], C["panel"]) if active else C["panel"]
        bc = C["accent"] if active else C["border"]
        drect_animated(surf, bg, (mcx, mcy, mw, mh), r=10, border=2 if active else 1, bcol=bc, hover=hovered, pulse=selected, frame=frame)
        dtext(surf, mname, F_HEAD, C["text"] if active else C["text2"], mcx+mw//2, mcy+8, "center", shadow=active, pulse=selected, frame=frame)
        for j, line in enumerate(mdesc.split("\n")):
            dtext(surf, line, F_SM, C["text3"], mcx+mw//2, mcy+30+j*17, "center")
        dtext(surf, mhint, F_SM, C["accent"] if active else C["text3"], mcx+mw//2, mcy+mh-20, "center", pulse=selected, frame=frame)

    bw2 = 200; bh2 = 50; bx2 = (W-bw2)//2; by2 = 495
    on2 = (hover == "start")
    grad_colors = (C["grad_start"], C["grad_end"])
    drect_animated(surf, grad_colors, (bx2, by2, bw2, bh2), r=12, border=2, bcol=C["accent"], 
                  hover=on2, pulse=on2, glow_color=C["glow_player"], frame=frame)
    dtext(surf, "🚀 START GAME", F_HEAD, (255,255,255), bx2+bw2//2, by2+bh2//2, "center", shadow=True, pulse=on2, frame=frame)
    dtext(surf, "or press  Enter", F_SM, C["text3"], W//2, by2+bh2+10, "center")
    drect_animated(surf, C["panel"], (W//2-210, 532, 420, 40), r=10, border=1, bcol=C["border2"], frame=frame)
    dtext(surf, "CURRENT SELECTION", F_SM, C["text3"], W//2, 540, "center")
    dtext(surf, f"{sel_diff} difficulty  •  {sel_mode.upper()} mode", F_UI, C["text"], W//2, 554, "center")

    shortcuts = [("Left / Right", "Select difficulty"), ("Tab", "Cycle mode"), ("Enter", "Start game")]
    drect_animated(surf, C["panel"], (W//2-300, 580, 600, 46), r=10, border=1, bcol=C["border"], frame=frame)
    dtext(surf, "QUICK START", F_SM, C["text3"], W//2, 588, "center")
    total_w = 24 * (len(shortcuts) - 1)
    badge_ws = []
    for k2, v2 in shortcuts:
        kw = F_SM.size(f"[{k2}]")[0] + 14
        vw = F_SM.size(v2)[0]
        badge_ws.append((kw, vw))
        total_w += kw + 12 + vw
    sx = (W - total_w) // 2
    sy = 602
    for (k2, v2), (kw, vw) in zip(shortcuts, badge_ws):
        drect_animated(surf, C["panel2"], (sx, sy, kw, 20), r=5, border=1, bcol=C["border2"], frame=frame)
        dtext(surf, f"[{k2}]", F_SM, C["accent"], sx+kw//2, sy+10, "center")
        dtext(surf, v2, F_SM, C["text3"], sx+kw+12, sy+10, "midleft")
        sx += kw + 12 + vw + 24

    hit_rects = {}
    for i in range(len(tw_cards)):
        hit_rects[f"card{i}"] = (20+i*(cw2+6), 130, cw2, ch2)
    for i, d in enumerate(diffs):
...
def draw_settings_panel(surf, s:GS, W, H, frame):
    if not s.show_settings:
        gear = settings_button_rect(W)
        pulse = frame % 80 < 18
        drect_animated(surf, C["panel2"], gear, r=6, border=1, bcol=C["border2"] if pulse else C["border"], pulse=pulse, frame=frame)
        dtext(surf, "⚙", F_UI, C["text3"], gear.centerx, gear.centery, "center", pulse=pulse, frame=frame)
        return
    panel = settings_panel_rect(W, H)
    pw, ph = panel.w, panel.h
    px, py = panel.x, panel.y
    if frame < 25:
        py += int(30 * (1 - frame/25))
    drect_animated(surf, C["panel"], (px, py, pw, ph), r=12, border=2, bcol=C["accent"], pulse=True, frame=frame)
    dtext(surf, "Settings", F_HEAD, C["accent"], px+pw//2, py+14, "center")
    dtext(surf, "Click any row to toggle an option.", F_SM, C["text3"], px+pw//2, py+30, "center")
    mouse_pos = pygame.mouse.get_pos()
    for row_rect, (label, attr, key, desc) in zip(settings_row_rects(W, H), SETTINGS_OPTIONS):
        enabled = getattr(s, attr)
        hovered = row_rect.collidepoint(mouse_pos)
        drect_animated(surf, C["panel2"], row_rect, r=10, border=1, bcol=C["accent"] if hovered else C["border"], hover=hovered, frame=frame)
        dtext(surf, f"{label} [{key}]", F_SM, C["text2"], row_rect.x+12, row_rect.y+7)
        dtext(surf, desc, F_SM, C["text3"], row_rect.x+12, row_rect.y+21)
        switch_rect = pygame.Rect(row_rect.right-62, row_rect.y+8, 50, 22)
        drect_animated(surf, C["green"] if enabled else C["pill_off"], switch_rect, r=11, border=1, bcol=C["border2"] if enabled else C["border"], frame=frame)
        knob_x = switch_rect.x + 11 + (28 if enabled else 0)
        pygame.draw.circle(surf, (255,255,255), (int(knob_x), switch_rect.y+11), 9)
    dtext(surf, "Press [S], click the gear, or click outside to close", F_SM, C["text3"], px+pw//2, py+ph-22, "center", pulse=True, frame=frame)
...
def apply_screen_shake(offset, intensity, duration):
    if duration <= 0:
        return (0, 0), 0
    new_offset = (random.randint(-intensity, intensity), random.randint(-intensity, intensity))
...
def gen_multi(s:GS):
        ...
        for rid in range(NUM_R):
            ...
            if r==n-1 and c==n-1 and not s.ma_done[rid]:
                ...
                if s.ma_winner<0:
                    ...
                    if s.particle_fx:
                        spawn_particles(s.particles, W//2 + random.randint(-100,100), H//2 + random.randint(-50,50), RCOLS[rid], count=20, spread=2.5, life=3.0)
    ...
    s.won=True
    s.ma_log.append(f"🏁 Race complete · Winner: R{s.ma_winner} · Paradoxes: {len(s.ma_paradox)}")
    yield "done"
...
def main():
    ...
    pygame.display.set_caption("✨ Temporal Maze — Local Multiplayer Edition ✨")
    ...
    SCOUT_MS  = 7; SOLVE_MS = 85; EXEC_MS = 115
    sel_diff = "Hard"; sel_mode = "player"; cur_scr = "title"
    state = GS(); ai_gen = None; ai_accum = 0.0
    frame = 0; hover_id = ""; title_hit_rects = {}
    shake_offset = (0, 0); shake_timer = 0
    ...
    # ✅ ADD pmove FUNCTION HERE ⬇️
    def pmove(dr, dc):
        """Handle single-player movement with time mechanics"""
        if state.won or state.mode != "player":
            return
    ...
    def mp_move(pid, dr, dc):
        if state.won or state.mode != "local_mp": return False
        player = state.mp_players[pid]
        if player["done"]: return False
        nr, nc, nt = player["r"]+dr, player["c"]+dc, (player["t"]+1)%state.T
        if walled(state.grid, state.temporal, state.n, state.T, nr, nc, nt):
            if dr == dc == 0:
                smsg(f"⚠️  {PLAYER_KEYS[pid]['name']}: Cannot wait here!", C["warn"])
            else:
                smsg(f"🚫 {PLAYER_KEYS[pid]['name']}: Blocked!", C["err"])
            return False
        if dr == dc == 0: player["waits"] += 1
        player["r"], player["c"], player["t"] = nr, nc, nt
        player["steps"] += 1
        state.view_t = nt  # Sync view to latest player action
        player["trail"].append((nr, nc))
        if len(player["trail"]) > 20: player["trail"].pop(0)
        # Check for win
        if nr == state.n-1 and nc == state.n-1 and not player["done"]:
            player["done"] = True
            if state.mp_winner < 0:
                state.mp_winner = pid
                state.won = True
                smsg(f"🏆 {PLAYER_KEYS[pid]['name']} WINS!", C["green"], 400)
                if state.particle_fx:
                    spawn_particles(state.particles, W//2, H//2, RCOLS[pid], count=50, spread=3.5, life=4.0)
                state.shake_timer = 20
            else:
                if pid not in state.mp_rankings:
                    state.mp_rankings.append(pid)
                smsg(f"✓ {PLAYER_KEYS[pid]['name']} finished!", C["text2"], 200)
        # Check for collisions with other players
        for other_pid in range(NUM_R):
            if other_pid != pid and not state.mp_players[other_pid]["done"]:
                op = state.mp_players[other_pid]
                if (player["r"], player["c"]) == (op["r"], op["c"]):
                    if (player["r"], player["c"]) not in state.mp_collisions:
                        state.mp_collisions.append((player["r"], player["c"]))
                        smsg(f"⚡ Collision! P{pid+1} & P{other_pid+1}", C["paradox"], 150)
                        state.shake_timer = 8
        return True
    ...
    def do_hint():
        ...
        if not path:
            ...
            state.show_hint = True; return
        twpts = [path[i][2] for i in range(1, len(path)) if path[i][:2] == path[i-1][:2]]
        if twpts:
            state.hint_txt = f"✨ Optimal path: {len(path)} steps with {len(twpts)} time wait(s).\nYou need to WAIT at time step(s): t={', t='.join(map(str, twpts))}\n\n💡 Strategy: navigate toward the blocked cell,\nthen press SPACE to wait — the wall opens at t+1.\n🔄 Remember: every move also advances time by 1!"
        else:
            state.hint_txt = f"✨ Optimal path: {len(path)} steps — NO time waits needed!\nThis is pure spatial navigation.\n💡 Tip: look for the open corridor leading toward\nthe bottom-right exit cell (red)."
    ...
    def launch_multi():
        ...
        state.ma_pos = list(robot_starts(state.n, state.grid, state.temporal, state.T))
    ...
    def reset_ai():
        ...
        state.path_set = set(); state.ma_phase = ""; state.show_end_page = False
        state.ma_pos = list(robot_starts(state.n, state.grid, state.temporal, state.T))
        state.ma_done = [False]*NUM_R; state.ma_winner = -1
        state.ma_trails = [[] for _ in range(NUM_R)]
        state.ma_paradox = []; state.ma_log = []; state.won = False

    state._h_prev = state.show_hint; state._p_prev = state.particle_fx
    state._f_prev = state.show_fps; state._d_prev = state.debug_mode

    while True:
        W, H = screen.get_size(); dt_ms = clock.tick(60); frame += 1; state.anim_tick = frame
        ...
        for ev in pygame.event.get():
            ...
            if cur_scr == "title":
                ...
                if ev.type == pygame.KEYDOWN:
                    ...
                    if ev.key == pygame.K_s: state.show_settings = not state.show_settings
                if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1:
                    if handle_settings_click(state, mx_g, my_g, W, H):
                        continue
                ...
                if ev.type == pygame.KEYDOWN:
                    ...
                    if k == pygame.K_r:
                        if state.mode in ("player", "local_mp"): new_level(state); ai_gen = None
                        else: reset_ai()
                    if k == pygame.K_n and state.won: new_level(state); ai_gen = None; reset_ai()
                    if k == pygame.K_e and state.won and state.mode in ("ai", "multi"):
                        state.show_end_page = not state.show_end_page
                    if k == pygame.K_LALT: state.show_settings = not state.show_settings
                    ...
                    if state.mode == "local_mp" and not state.won:
                        ...
                        for ti in range(state.T):
                            if k == getattr(pygame, f"K_{ti}", None):
                                if state.view_t != ti: state.target_view_t = ti; state.view_transition = 1.0
                    elif state.mode == "ai":
                    ...
                    if state.mode == "player" and not state.won:
                        ...
                        if 0 <= cr2 < state.n and 0 <= cc2 < state.n:
                            ...
                            if abs(dr3) + abs(dc3) == 1:
                                pmove(int(math.copysign(1, dr3)) if dr3 else 0, int(math.copysign(1, dc3)) if dc3 else 0)
                            elif dr3 == dc3 == 0: pmove(0, 0)

        if cur_scr == "title":
            hover_id = ""
            for hid, (hx, hy, hw, hh2) in title_hit_rects.items():
                if hx <= mx_g <= hx+hw and hy <= my_g <= hy+hh2: hover_id = hid; break

        if cur_scr == "game" and state.ai_running and ai_gen:
            ph = state.ma_phase
            tick = (SCOUT_MS if ph in ("", "scout", "gossip", "negotiate") else EXEC_MS if ph == "execute" else SOLVE_MS)
            ai_accum += dt_ms
            if ai_accum >= tick:
                ai_accum = 0
                try: next(ai_gen)
        ...
        if cur_scr == "title":
            ...
            mah = H - HUD_H - BOT_H; maze_x0 = SIDEBAR_W + 4; maze_w = W - SIDEBAR_W - 8
            cell = max(18, min(mah//state.n, maze_w//state.n))
            ox = maze_x0 + (maze_w - cell*state.n)//2; oy = HUD_H + (mah - cell*state.n)//2
            drect_animated(render_surf, C["panel"], (0, HUD_H, SIDEBAR_W, mah), r=0, border=1, bcol=C["border"], frame=frame)
            draw_legend(render_surf, 4, HUD_H+6, SIDEBAR_W-8, frame)
            leg_h = len(LEGEND_ITEMS)*24 + 28; ctrl_y = HUD_H + leg_h + 14
            draw_controls_panel(render_surf, 4, ctrl_y, SIDEBAR_W-8, state, frame)
            ctrl_h = len(get_control_rows(state))*24 + 32
            if state.mode == "multi" and state.ma_phase:
                draw_robot_panel(render_surf, 4, ctrl_y+ctrl_h+8, SIDEBAR_W-8, state, frame)
            elif state.mode == "local_mp":  # 🎮 Draw multiplayer player panel
                draw_mp_player_panel(render_surf, 4, ctrl_y+ctrl_h+8, SIDEBAR_W-8, state, frame)
            draw_hud(render_surf, state, W, frame)
            draw_maze(render_surf, state, ox, oy, cell, frame)
            draw_statusbar(render_surf, state, W, H, frame)
            if state.ma_log:
                log_y = oy + cell*state.n + 4; log_w = W - SIDEBAR_W - 10
                if H - log_y - BOT_H > 40: draw_log_panel(render_surf, state, maze_x0, log_y, log_w, frame)
            if not state.ai_running and not state.won:
                prompt = {"ai": "Press  [A]  to run AI solver ✨", "multi": "Press  [A]  to start multi-agent race 🏁", 
                         "local_mp": "🎮 3 Players Ready! Press keys to race!"}.get(state.mode, "")
                if prompt:
                    pw2 = F_HEAD.size(prompt)[0] + 28; px2 = ox + (cell*state.n - pw2)//2; py2 = oy + cell*state.n//2 - 18
                    drect_animated(render_surf, C["panel2"], (px2, py2, pw2, 36), r=8, border=1, bcol=C["accent"], pulse=True, frame=frame)
            ...
            if state.debug_mode:
                debug_info = [f"View T: {state.view_t}/{state.T}", f"Transition: {state.view_transition:.2f}", f"Particles: {len(state.particles)}"]
                for i, info in enumerate(debug_info): dtext(render_surf, info, F_SM, C["text3"], 10, 20+i*16)
