...
def draw_controls_panel(surf, x, y, W_panel, mode, frame):
    ...
    elif mode == "local_mp":  # 🎮 NEW: Local Multiplayer controls
        rows = [
            (f"{RNAMES[0]}", "[WASD] + [SPACE]"),
            (f"{RNAMES[1]}", "[Arrows] + [RSHIFT]"),
            (f"{RNAMES[2]}", "[IJKL] + [ENTER]"),
            ("Restart",   "[R]"),
            ("Menu",      "[ESC]"),
        ]
    else:
    ...
    for i,(action,keys) in enumerate(rows):
        ry=y+20+i*20
        col = C["accent"] if mode == "local_mp" and i < 3 else C["text3"]
        dtext(surf, action+":", F_SM, col, x+8, ry)
        dtext(surf, keys, F_SM, C["accent"] if i < 3 else C["text2"], x+W_panel-8, ry, "topright")
...
def draw_statusbar(surf, s:GS, W, H, frame):
    ...
    if s.msg and s.msg_timer > 0:
        ...
        elif s.mode == "local_mp":  # 🎮 Multiplayer hints
            parts = ["🎮 3 Players: WASD | Arrows | IJKL", "[R] Restart", "[ESC] Menu"]
        else:
            parts = ["[A] Run race", "[R] Reset", "[ESC] Menu"]
        hint = " · ".join(parts)
...
def draw_title(surf, W, H, sel_diff, sel_mode, hover, frame):
    ...
    dtext(surf, "✨ Multi-Agent + Local Multiplayer ✨", F_UI, C["text3"], W//2, 102, "center", pulse=True, frame=frame)
    ...
    for i, (icol, title2, body) in enumerate(tw_cards):
        ...
        drect_animated(surf, bg if isinstance(bg, tuple) and len(bg)==2 else C["panel2"], 
                      (cx2, cy2, cw2, ch2), r=10, border=2 if on else 1, bcol=bc, hover=on, frame=frame)
    ...
    for i, d in enumerate(diffs):
        ...
        drect_animated(surf, bg if isinstance(bg, tuple) and len(bg)==2 else C["panel"], 
                      (dcx, dcy, dcw, dch), r=10, border=2 if on else 1, bcol=bc, hover=on, pulse=on, frame=frame)
        info = DIFFS[d]
        dtext(surf, d, F_HEAD, C["text"] if on else C["text2"], dcx+dcw//2, dcy+8, "center", shadow=on, pulse=on, frame=frame)
    ...
    for i, (m, mname, mdesc, mhint) in enumerate(modes):
        ...
        bc = C["accent"] if on else C["border"]
        drect_animated(surf, bg if isinstance(bg, tuple) and len(bg)==2 else C["panel"], 
                      (mcx, mcy, mw, mh), r=10, border=2 if on else 1, bcol=bc, hover=on, pulse=on, frame=frame)
    ...
    shortcuts = [("← →", "Select difficulty"), ("Tab", "Cycle mode"), ("Enter", "Start game")]
    sy = 560
...
def main():
    ...
    while True:
        ...
        for ev in pygame.event.get():
            ...
            if cur_scr == "title":
                ...
                if ev.type == pygame.KEYDOWN:
                    ...
                    if k == pygame.K_s: state.show_settings = not state.show_settings
                ...
                if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1:
                    ...
                    if state.show_settings:
                        pw, ph = 340, 260; px, py = (W-pw)//2, (H-ph)//2 - 20
                        if not (px <= mx_g <= px+pw and py <= my_g <= py+ph): state.show_settings = False
                            ...
                            for i, (label, enabled, key, desc) in enumerate([
                                ...
                                y_pos = options_y + i * 38
                                switch_x = px + pw - 70 - 180
        ...
        if cur_scr == "title":
            ...
            leg_h = len(LEGEND_ITEMS)*22 + 18; ctrl_y = HUD_H + leg_h + 14
            draw_controls_panel(render_surf, 4, ctrl_y, SIDEBAR_W-8, state.mode, frame)
            ctrl_rows = {"player": 6, "ai": 4, "multi": 4, "local_mp": 5}
